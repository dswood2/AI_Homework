...
def constructBayesNet(gameState: hunters.GameState):
    ...
    max_distance = max(manhattanDistance((0, 0), (X_RANGE - 1, Y_RANGE - 1)),
                       manhattanDistance((X_RANGE - 1, Y_RANGE - 1), (0, 0))) + MAX_NOISE
    variableDomainsDict[OBS0] = list(range(max_distance + 1))
    variableDomainsDict[OBS1] = list(range(max_distance + 1))
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Multiply all remaining factors and normalize to get the final joint distribution
        finalJointDistribution = joinFactors(factors)
        finalJointDistribution = normalize(finalJointDistribution)
        return finalJointDistribution
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Initialize the new belief distribution
        newBeliefs = DiscreteDistribution()

        # Calculate the normalization factor (P(Observation))
        ...
        for position in self.allPositions:
            ...
            normalizingConstant += obsProb * prior

        # Handle the case where normalizingConstant is zero
        if normalizingConstant == 0:
            self.beliefs = DiscreteDistribution()
            self.beliefs.normalize()  # Uniform distribution over all positions
            return
        ...
        for position in self.allPositions:
            obsProb = self.getObservationProb(observation, pacmanPosition, position, jailPosition)
            prior = self.beliefs[position]
            posterior = obsProb * prior / normalizingConstant
            newBeliefs[position] = posterior
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        # Handle the case when numParticles is not divisible by numPositions
        remainingParticles = self.numParticles % numPositions
        self.particles.extend(random.sample(self.legalPositions, remainingParticles))
        "*** END YOUR CODE HERE ***"
